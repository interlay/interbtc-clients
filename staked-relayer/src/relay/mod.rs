...
pub use issuing::Client as InterBtcClient;

use crate::core::{Error as CoreError, Runner};
use service::Error as ServiceError;

pub async fn run_relayer(runner: Runner<Error, BitcoinClient, InterBtcClient>) -> Result<(), ServiceError> {
    loop {
        match runner.submit_next().await {
            Ok(_) => (),
            Err(CoreError::Issuing(Error::InterBtcError(ref err))) if err.is_duplicate_block() => {
                tracing::info!("Attempted to submit block that already exists")
            }
            Err(CoreError::Issuing(Error::InterBtcError(ref err))) if err.is_rpc_disconnect_error() => {
