...
pub async fn run_relayer(runner: Runner<BitcoinCore, PolkaBtcProvider>) -> Result<(), ServiceError> {
    loop {
        match runner.submit_next().await {
            Ok(_) => (),
            Err(Error::PolkaBtcError(ref err)) if err.is_duplicate_block() => {
                tracing::info!("Attempted to submit block that already exists")
            }
            Err(Error::PolkaBtcError(ref err)) if err.is_rpc_disconnect_error() => {
