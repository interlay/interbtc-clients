...
const BOB_BTC_ADDRESS: &str = "BF3408F6C0DEC0879F7C1D4D0A5E8813FC0DB569";
const VAULT_COLLATERAL: u128 = 100;
...
async fn main() -> Result<(), Error> {
    ...
    // TODO: Error: XtError(Metadata(ErrorNotFound(0)))
    ...
    // VAULT REGISTRY
    let vault_prov = bob_prov.clone();
    let bob_address = H160::from_slice(
        hex::decode(BOB_BTC_ADDRESS)
                .unwrap()
                .as_slice(),
        ); 
    vault_prov.register_vault(VAULT_COLLATERAL, bob_address).await?;
